...
func validateInput(a, b int, useRoman bool) error {
	if useRoman && (a < 1 || a > 10 || b < 1 || b > 10) {
...
func calculator(input string) (string, error) {
	...
	count := strings.Count(input, matches[2])
	if count > 1 {
		return "", fmt.Errorf("допустим только один оператор")
	}
	...
	err := validateInput(a, b, useRoman)
	if err != nil {
		return "", err
	}
